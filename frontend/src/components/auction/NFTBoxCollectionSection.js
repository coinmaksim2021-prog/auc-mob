...
const NFTBoxCollectionSection = () => {
  ...
  return (
    <section className="mt-12 mb-12 py-12" data-testid="nft-box-collection">
      <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex gap-4 px-2">
            {nftBoxes.map((nft) => {
              ...
              return (
                <div 
                  key={nft.token_id}
                  className="flex-shrink-0 w-48 group rounded-3xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:scale-[1.02]"
                  style={{ 
                    background: '#F5FBFD',
                ...
                >
                  <div className="relative">
                    <img
                      ...
                      className="w-full aspect-square object-cover rounded-t-telegram-xl"
                      onError={(e) => {
                        e.target.src = 'https://via.placeholder.com/300x300/000000/00FF87?text=F';
                      }}
                    />
                    <div className={`absolute top-2 right-2 ${rarity.bg} backdrop-blur-sm px-2.5 py-1 rounded-xl shadow-sm`}>
                      <span className={`text-xs font-semibold ${rarity.color}`}>{rarity.name}</span>
                    </div>
                    <div className="absolute bottom-2 left-2 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-xl shadow-sm">
                      <span className="text-xs font-bold text-gray-900">#{nft.token_id}</span>
                    </div>
                  </div>
                  
                  <div className="p-3 bg-white rounded-b-telegram-xl">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-5 h-5 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-xl flex items-center justify-center">
        ...
        <div className="mt-8 text-center">
          ...
          <a 
            ...
            className="inline-flex items-center gap-2 px-8 py-4 bg-gray-900 text-white rounded-3xl font-semibold hover:bg-black transition-all shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95"
          >
