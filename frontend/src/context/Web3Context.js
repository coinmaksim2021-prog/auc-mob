...
export function Web3Provider({ children }) {
  ...
  // Get Dynamic context - always call hook
  const { primaryWallet } = useDynamicContext();

  // Sync wallet address from Dynamic Labs
  useEffect(() => {
    if (primaryWallet?.address) {
      console.log('=== Dynamic wallet detected:', primaryWallet.address);
      setWalletAddress(primaryWallet.address);
      
      // Also check network when Dynamic wallet connects
      if (typeof window !== 'undefined' && window.ethereum) {
        checkNetwork();
      }
    } else if (!primaryWallet && walletAddress) {
      // Dynamic wallet disconnected
